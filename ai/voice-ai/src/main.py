...
def create_app() -> FastAPI:
    ...
    event_pub = KafkaProducer(brokers=config.VOICE_KAFKA_BROKERS)
    service = VoiceService(repo=repo, event_pub=event_pub)

    @asynccontextmanager
    async def lifespan(app: FastAPI):
        logger.info("Voice AI Service starting on port %d", config.VOICE_PORT)
        yield
