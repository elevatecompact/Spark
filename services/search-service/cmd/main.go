...
import (
	...
	"github.com/elevatecompact/spark/services/search-service/internal/config"
	"github.com/elevatecompact/spark/services/search-service/internal/events"
	"github.com/elevatecompact/spark/services/search-service/internal/handler"
...
func main() {
	...
	engine := repository.NewNoopSearchEngine()
	...
	consumer := events.NewNoopConsumer()
	go func() {
		if err := consumer.Start(ctx); err != nil {
			log.Warn().Err(err).Msg("consumer stopped")
		}
	}()

	svc := service.NewSearchService(engine, repo, cache, eventPub)
	h := handler.New(svc)
	...
	go func() {
		log.Info().Str("port", cfg.Port).Msg("search-service starting")
