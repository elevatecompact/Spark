...
import (
	...
	"github.com/elevatecompact/spark/services/competition-service/internal/config"
	"github.com/elevatecompact/spark/services/competition-service/internal/events"
	"github.com/elevatecompact/spark/services/competition-service/internal/handler"
...
func main() {
	...
	repo := repository.NewCompetitionRepository(pool)
	...
	svc := service.NewCompetitionService(repo, eventPub)
	h := handler.New(svc)
	...
	go func() {
		log.Info().Str("port", cfg.Port).Msg("competition-service starting")
