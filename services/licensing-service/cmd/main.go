...
import (
	...
	"github.com/elevatecompact/spark/services/licensing-service/internal/config"
	"github.com/elevatecompact/spark/services/licensing-service/internal/domain"
	"github.com/elevatecompact/spark/services/licensing-service/internal/events"
	"github.com/elevatecompact/spark/services/licensing-service/internal/handler"
...
func main() {
	...
	repo := repository.NewLicensingRepository(pool)

	var eventPub domain.EventProducer
	...
	svc := service.NewLicensingService(repo, eventPub)
	h := handler.New(svc)
	...
	go func() {
		log.Info().Str("port", cfg.Port).Msg("licensing-service starting")
