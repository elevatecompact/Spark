...
import (
	...
	"github.com/elevatecompact/spark/services/trust-service/internal/config"
	"github.com/elevatecompact/spark/services/trust-service/internal/domain"
	"github.com/elevatecompact/spark/services/trust-service/internal/events"
	"github.com/elevatecompact/spark/services/trust-service/internal/handler"
...
func main() {
	...
	repo := repository.NewTrustRepository(pool)
	...
	svc := service.NewTrustService(repo, eventPub)
	h := handler.New(svc)
	...
	go func() {
		log.Info().Str("port", cfg.Port).Msg("trust-service starting")
