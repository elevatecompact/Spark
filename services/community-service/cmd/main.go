...
import (
	...
	"github.com/elevatecompact/spark/services/community-service/internal/config"
	"github.com/elevatecompact/spark/services/community-service/internal/events"
	"github.com/elevatecompact/spark/services/community-service/internal/handler"
...
func main() {
	...
	repo := repository.NewCommunityRepository(pool)
	...
	svc := service.NewCommunityService(repo, eventPub)
	h := handler.New(svc)
	...
	go func() {
		log.Info().Str("port", cfg.Port).Msg("community-service starting")
