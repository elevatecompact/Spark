...
import (
	...
	"github.com/elevatecompact/spark/services/advertising-service/internal/config"
	"github.com/elevatecompact/spark/services/advertising-service/internal/events"
	"github.com/elevatecompact/spark/services/advertising-service/internal/handler"
...
func main() {
	...
	repo := repository.NewAdvertisingRepository(pool)
	...
	svc := service.NewAdvertisingService(repo, eventPub)
	h := handler.New(svc)
	...
	go func() {
		log.Info().Str("port", cfg.Port).Msg("advertising-service starting")
